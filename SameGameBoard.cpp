...
int CSameGameBoard::DeleteBlocks(int row,int col)
{
	...
	{
		//  Then call the recursive function to eliminate all other tourch blocks with the same color
		m_arrBoard[row][col]=0;
		nCount = 1;
	...
	//  Return the total number of pieces of delete
	return nCount;
}
...
int CSameGameBoard::DeleteNeighborBlocks(int row,int col,int color, Direction direction)
{
	...
	//  If we weren't told to not go back lefy,check left
...
void CSameGameBoard::CompactBoard(void)
{
	...
	for(int col=0;col<m_nColumns;col++)
	{
		...
		while(nNextEmptyRow>=0 && nNextOccupiedRow>=0)
		{
